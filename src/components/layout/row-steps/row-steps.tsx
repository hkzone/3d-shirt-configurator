...
export const RowSteps = React.forwardRef<HTMLButtonElement, RowStepsProps>(
  (
    ...
    return (
      <nav aria-label='Progress' className='max-w-fit overflow-x-auto max-sm:mx-auto scrollbar-hide'>
        <ol className={cn('flex flex-row flex-nowrap gap-x-3', colors, className)}>
          {steps?.map((step, stepIdx) => {
            ...
            return (
              <li key={stepIdx} className='relative flex w-full items-center pr-6 sm:pr-12'>
                <button
                  key={stepIdx}
                  ref={ref}
                ...
                >
                  <div className='h-ful relative flex items-center'>
                    <LazyMotion features={domAnimation}>
                      <m.div animate={status} className='relative'>
                        <m.div
                          className={cn(
                            'relative flex max-lg:h-[24px] max-lg:w-[24px] max-lg:!text-small',
                            'h-[34px] w-[34px] items-center justify-center rounded-full border-medium text-large font-semibold text-default-foreground',
                            {
                        ...
                        >
                          <div className='flex items-center justify-center'>
                            {status === 'complete' ? (
                              <CheckIcon className=' max-lg:h-4 max-lg:w-4 h-6 w-6 text-[var(--active-fg-color)]' />
                            ) : (
                              <span>{stepIdx + 1}</span>
                            )}
                          </div>
                  ...
                  <div className='max-w-full flex-1 text-start'>
                    <div
                      className={cn(
                        'text-xs sm:text-small font-medium text-default-foreground transition-[color,opacity] duration-300 group-active:opacity-80 lg:text-medium',
                        {
                  ...
                  {stepIdx < steps.length - 1 && !hideProgressBars && (
                    <div
                      aria-hidden='true'
                      className='pointer-events-none absolute right-0  w-4 sm:w-10 flex-none items-center'
                      style={{
                        // @ts-expect-error
                        '--idx': stepIdx,
                      }}
